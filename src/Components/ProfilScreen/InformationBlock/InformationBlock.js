...
import { setDataToDatabase } from "../../../data/FirebaseUtils";
...
export default class adressblock extends Component {
    ...
    confirmEdit = () => {
        if (this.props.txt === "Phone") {
            setDataToDatabase(`users/${this.props.id}/phone`, {
                phone: this.state.mobilePhoneInput,
            }).catch((error) => {
                console.log(error);
            });
        } else if (this.props.txt === "Email") {
            setDataToDatabase(`users/${this.props.id}/email`, {
                email: this.state.mobilePhoneInput,
            }).catch((error) => {
                console.log(error);
            });
        }
                    ...
                    {!this.props.isEditMode ? (
                        ...
                        <View style = {{width: "100%", display: 'flex', flexDirection: 'row', alignItems: 'center'}}>
                            <TextInput
                                ...
                                keyboardType={
                                    this.props.txt === "Phone" ? "phone-pad" : "email-address"
                                }
                                onChangeText={(text) => this.setState({mobilePhoneInput: text})}
                            />
                            <TouchableOpacity onPress={() => this.confirmEdit()} style={{marginLeft: "2%", width: 20, height: 20}}>
